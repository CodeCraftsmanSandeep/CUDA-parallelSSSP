        ...
        bool*                __restrict__ relaxed)
{
    ...
    while(__builtin_expect(u < numNodes, 1))
    {
        if(currFlag[u])
        {
            ...
            for(int outEdge = rowPtr[u]; outEdge < rowPtr[u+1]; outEdge++)
            {
                v = colInd[outEdge];
                w = weights[outEdge];
      ...
      bool* __restrict__ d_flagB) 
{
    ...
    for(int iter = 1; (iter <= numNodes) && relaxed; iter++)
    {
        ...
        if(iter & 1){
            relaxEdges <<< 256, 256 >>> (numNodes, d_rowPtr, d_colInd, d_weights, d_dist, d_flagA, d_flagB, d_relaxed);
        }else{
            relaxEdges <<< 256, 256 >>> (numNodes, d_rowPtr, d_colInd, d_weights, d_dist, d_flagB, d_flagA, d_relaxed);
        }
